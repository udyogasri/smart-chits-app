...
function DashboardPage() {
  ...
  const [memberStatus, setMemberStatus] = useState({}) // track membership for each chit

  // Fetch chits from backend on component mount
  useEffect(() => {
    const fetchChits = async () => {
      try {
        ...
        // Check membership status for all chits
  ...
  const handleConfirmJoin = async () => {
    ...
    setIsJoining(true)
    try {
      const result = await joinChit(selectedChit.id)
  ...
  const getJoinButtonState = (chit) => {
    ...
    const isFull = (chit.current_members || chit.members || 0) >= (chit.total_members || 20)
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && (
        <>
          <div>
            <h1 className="text-3xl font-bold text-white">{t('listOfChits') || 'List of Chits'}</h1>
            <p className="text-slate-400 mt-2">Browse and manage all available chit fund plans</p>
          </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1">
          <input
            type="text"
            placeholder={t('searchChits') || 'Search chits by name, organizer, or description...'}
            value={searchTerm}
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-700">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">{t('chitName') || 'Chit Name'}</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">{t('amount') || 'Amount'}</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">{t('duration') || 'Duration'}</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">{t('members') || 'Members'}</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">{t('monthly') || 'Monthly'}</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">{t('status') || 'Status'}</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">{t('actions') || 'Actions'}</th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={chit.id} className="hover:bg-slate-700/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-300">
                      {(chit.current_members || chit.members || 0)}/{chit.total_members || 20}
                    </td>
      ...
      {filteredChits.length === 0 && (
        <div className="text-center py-12">
          <p className="text-slate-400 text-lg">{t('noChitsFound') || 'No chits found matching your criteria'}</p>
        </div>
      )}
